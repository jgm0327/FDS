 ...
 * 독립된 State Store 엔트리(키 = 샤드 전체 문자열)를 갖는다 — {@link AccountActivityState}(옛
 * 단일 계좌 상태)와 달리 lastTransactionTimestamp/lastCountry는 여기서 관리하지 않는다. 그 값은
 * "전역에서 가장 최근"이어야 하는데 샤드 하나만 보고는 알 수 없고, Stage 2({@link
 * AccountActivityMergeProcessor})가 여러 샤드를 병합해서 계산한다.
 */
@Data
@NoArgsConstructor
