...
- **애매한 구간 가중치/임계값 산정** — 원래 "모델이 시퀀스 전체를 보니 규칙보다 믿을 만하다"는
  ...
  - **실측 결과**(2026-09-11, `docs/sessions/2026-09-11_ai-ensemble-weight-tuning_session-01.md`
    ...
    0.0880 대비, 그리드서치로 찾은 값(모델weight=1.0/규칙weight=0.0, low=0.20/high=0.75)의
    기대 비용은 0.0633 — 약 28% 낮음. 사기 완전차단율도 93.35% → 96.67%로 개선, 정상거래
    마찰율(추가인증+차단)은 1.23% → 1.27%로 거의 그대로.
  - **왜 규칙weight=0이 안전한가**: `ModelInferenceClient`가 서킷브레이커 오픈/타임아웃 시
    반환하는 폴백 확률 자체가 `RuleBasedFallbackScorer`의 출력이다(`FraudScore.SOURCE_FALLBACK`)
    — 즉 규칙 신호는 "가중합의 한 항"이 아니라 "모델 장애 시의 대체 입력"으로 시스템에 여전히
    남아있다. 규칙weight를 0으로 낮춰도 이 대체 경로가 사라지는 게 아니라서 안전하다고 판단.
