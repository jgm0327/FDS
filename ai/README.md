...
| 그리드서치 산정값 | 0.95 : 0.05 | 0.25/0.45 | 0.0817 | 1.25% | 98.34% | 0.95% |

기대 비용 약 34% 감소, 사기 완전차단율 개선(93.35%→98.34%), 정상거래 마찰율도 소폭 개선
(1.23%→0.95%) — 대신 사기 완전누출율은 1.04%→1.25%로 소폭 올랐다("추가인증으로만 거르는"
애매한 판정보다 "완전 차단/완전 허용"으로 더 확실히 가르는 쪽이 이 비용 가정 하에서 총비용이
낮다는 결과). 전체 confusion 표는
`docs/sessions/2026-09-11_ai-ensemble-weight-tuning_session-01.md` 참고.

**코드 리뷰가 잡아낸 비용 가정 버그**: 처음 이 표에 적었던 값(모델:규칙=1.0:0.0, 기대 비용
0.0633)은 `CostWeights`의 `fraud_step_up`(4.0)이 `normal_block`(6.0)보다 작게 설정된 채로 나온
결과였다 — docstring에 적어둔 의도("사기를 추가인증으로만 거르는 게 정상거래를 차단하는 것보다
나쁘다")와 실제 숫자가 반대였다. `/code-review`가 지적해서 `fraud_step_up=8.0`으로 바로잡고
그리드서치를 재실행한 게 위 표의 값이다.

**규칙weight가 완전히 0이 아니라 0.05인 이유**: `backend/model-client`의
`ModelInferenceClient`가 서킷브레이커 오픈/타임아웃 시 반환하는 폴백 확률 자체가
`RuleBasedFallbackScorer`의 출력이다 — 즉 규칙 신호는 정상 상황에서도 가중합의 작은 한 항으로,
장애 시에는 대체 입력으로 시스템에 이중으로 반영된다. 이 결과는 합성 데이터 분포와 스크립트가
가정한 비용표 위에서 나온 것이라, 실제 라벨 데이터가 쌓이면 재검증이 필요하다 — 이 CostWeights
가정 자체를 실제 비용으로 재추정하는 것이 다음 개선 과제다.
